...
import type { MobilityLastSession } from '@/types/mobility';
...
const Mobility = () => {
  ...
  const movements = useMemo(() => foldMovements(data?.items ?? []), [data?.items]);
  ...
  const handleTransfer = async () => {
    ...
    await transfer.mutateAsync(format(transferDate, 'yyyy-MM-dd'));
        ...
        <div className="page__content page__content--narrow">
          <div className="page__header animate-in">
            ...
            <p className="page__subtitle">
              One rolling routine, adjusted from the last session
            </p>
          </div>
          ...
          {isLoading ? (
            <div className="text-center" style={{ padding: 'var(--space-16) 0' }}>
              ...
              <p style={{ marginTop: 'var(--space-4)', color: 'var(--text-muted)' }}>
                Loading mobility session...
              </p>
            </div>
          ) : data?.ready ? (
            /* State 1 — a session is waiting to be run. */
            <div
                  ...
                  >
                    <div>
                      ...
                      >
                        ...
                        {data.generated_at
                          ? ` · written ${format(new Date(data.generated_at), 'MMM d')}`
                          : ''}
                      </p>
                    </div>
