...
def get_exercises_by_category():
    ...
    for workout in workouts:
        ...
        if category and exercise:
            ...
            if (
                exercise not in exercise_dates[category]
                or date > exercise_dates[category][exercise]
            ):
...
def reorder_workout(target_date, workout_index, direction):
    ...
    if direction == "up":
        date_workouts[workout_index], date_workouts[workout_index - 1] = (
            date_workouts[workout_index - 1],
            date_workouts[workout_index],
        )
    else:  # down
        date_workouts[workout_index], date_workouts[workout_index + 1] = (
            date_workouts[workout_index + 1],
...
def get_lowest_session_index():
    ...
    sessions = [
        int(w.get("Session", 0)) for w in workouts if w.get("Session", "").isdigit()
    ]
...
def pop_upcoming_workout_session(target_date):
    ...
    remaining_workouts = [
        w for w in upcoming if w.get("Session") != str(lowest_session)
    ]
...
def get_progression_data(exercise_name):
    ...
    for workout in historical_raw:
        ...
        if (
            date not in historical_by_date
            or estimated_1rm > historical_by_date[date]["estimated_1rm"]
        ):
    ...
    for session_num in sorted(
        all_sessions.keys(), key=lambda x: int(x) if x.isdigit() else 0
    ):
    ...
    return {"historical": historical, "upcoming": upcoming}
