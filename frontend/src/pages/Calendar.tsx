...
const Calendar = () => {
  ...
  const { data: calendarData, isLoading } = useQuery({
    ...
    queryFn: () => workoutsApi.getCalendar(currentYear, currentMonth),
  });
  ...
  const workoutCounts = calendarData?.data?.counts || {};
    ...
    days.push(
      <div
        ...
        className={`h-20 sm:h-24 border border-border rounded-lg p-2 cursor-pointer transition-colors ${
          count > 0 ? 'bg-green-900/20 hover:bg-green-900/30' : 'hover:bg-accent'
        } ${isToday ? 'ring-2 ring-primary' : ''}`}
      >
        <div className="font-semibold text-sm sm:text-base">{day}</div>
        {count > 0 && (
          <div className="text-xs text-green-400 mt-1">{count} workout{count > 1 ? 's' : ''}</div>
        )}
      </div>
    );
  }
